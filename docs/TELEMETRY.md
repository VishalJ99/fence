...
| E13 | `support.diagnostic_snapshot` | Fence menu → Send Diagnostic Report Now while global Fence telemetry consent is enabled | info | `collector_status`, `last_strictify_outcome`, UI/model/render match booleans | Persisted/decoded/model/rendered calendar counts, view attachment/area state, daemon reachability, settings/physical-layer state and match booleans; the returned event ID is shown as a `FENCE-` support reference. One-shot consent remains a §2.2 gate |
...
Fence menu → Send Diagnostic Report Now requests one current sanitized daemon snapshot, combines it with bounded persisted/decoded app counts and actual calendar presentation counters, captures `support.diagnostic_snapshot` (E13), flushes Sentry, and shows `FENCE-<first 8 of Sentry event ID>`. It requires global Fence telemetry consent and never creates a local log file, attachment, or email. Support capture does not tag or drain historical spool records.
