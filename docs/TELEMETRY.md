...
| Support | Report Bug captures a sanitized structural snapshot when globally opted in and puts `FENCE-<event-id-prefix>` in the support email/log header; email log files are protected and scrubbed |
...
| E13 | `support.diagnostic_snapshot` | Existing Report Bug/log-export flow while global Fence telemetry consent is enabled | info | `collector_status`, `last_strictify_outcome` | Basic app/daemon structural counts, reachability, settings/physical-layer state and match booleans; event ID is cross-referenced in the support email. Full one-shot/E1-style support capture remains a §2.2 gate |
