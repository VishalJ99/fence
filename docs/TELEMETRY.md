...
| E10 | `emergency.unlock_result` | Emergency script returns fixed root-side settings/hosts/PF postconditions and the UI changes state only after verified cleanup | info (verified success) / error | `outcome` (`success`\|`script_error`\|`verify_failed`) | AppleScript error code, `{settings_cleared, hosts_clean, pf_check}`, duration |
...
8. **On-disk/support-log hygiene:** delete `/tmp` debug logs and blocklist/authorization/device/settings-printing NSLogs. Move schedule stdout/stderr to a protected Fence log directory or disable them. There is no support-log export path: the manual support action serializes one typed allowlisted event and never reads, attaches, or sends unified-log excerpts.
...
Fence menu → Send Diagnostic Report Now sends the app-derived expected schedule projection across the authenticated local XPC boundary, receives only aggregate equality/delta results, and combines them with bounded persisted/decoded app counts and actual calendar-presentation counters. The UI counters distinguish model rows, created block views, nonzero geometry, intersection with the calendar, nontransparent appearance, and effective visibility without uploading block frames, times, colors, or content. Fence then captures `support.diagnostic_snapshot` (E13), flushes Sentry, and shows `FENCE-<first 8 of Sentry event ID>`. It requires global Fence telemetry consent and never creates a local log file, attachment, screenshot, or email. Support capture does not tag or drain historical spool records.
...
- **3.4.9 consent-menu release (closed):** build 649 embeds the production DSN, its `C736846D-8CE5-3E4B-93ED-5545906FD3FC` dSYM was uploaded, the signed app passed the launch/profile/entitlement smoke checks, and notarized ZIP/DMG artifacts were published through both Sparkle appcasts and the website download links.
- **3.4.10 Sentry-only support release (closed):** build 650 embeds the production DSN; dSYM UUID `29A783D9-454C-3B91-9730-F75A936ADF4A` is present in Sentry; Direct Distribution profile UUID `002a5081-d4ff-4b9c-bf96-0cb18ffd3679` matches the release certificate; the signed app passed the restricted-entitlement launch smoke; and notarized ZIP/DMG artifacts were published through both Sparkle appcasts and the website. Public SHA-256 values are `421ec2e92abf625ee308574a11d46516b7e046d96ad56168e9ff746977a837b6` (ZIP) and `bb38e4d6102d9758bda5cf2680593f7635a2013d46b654de7366cb70903e293c` (DMG).
- **3.4.11 dense support-snapshot release (closed):** build 651 adds effective calendar-block visibility stages and exact aggregate daemon schedule drift to the manual report; dSYM UUID `E7098E1D-FF39-3DBD-B0B7-A182CB11E87B` is present in Sentry; Direct Distribution profile UUID `002a5081-d4ff-4b9c-bf96-0cb18ffd3679` matches the release certificate; the signed app passed the restricted-entitlement launch smoke; and notarized ZIP/DMG artifacts were published through both Sparkle appcasts and the website. Public SHA-256 values are `27269d68ac2e003839d7706cc3707f8fb62d84f4b3a0228e50f56e1959ebc6e8` (ZIP) and `85618ed3903f3b21fc8055cc6ef318a707f71b24fea6b516bd56e2e973029fb0` (DMG).
